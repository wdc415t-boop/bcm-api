...
const stmts = {
  createUser: db.prepare(`
    INSERT INTO users (id, email, password_hash, name, home_region) VALUES (?, ?, ?, ?, ?)
  `),
  getUserByEmail: db.prepare(`SELECT * FROM users WHERE email = ?`),
  getUserById: db.prepare(`SELECT * FROM users WHERE id = ?`),
  updateUser: db.prepare(`
    UPDATE users SET name = ?, home_region = ?, avatar_url = ?, updated_at = datetime('now') WHERE id = ?
  `),
  ...
  updateSubscriptionStripe: db.prepare(`
    UPDATE subscriptions SET stripe_customer_id = ?, stripe_subscription_id = ?, status = ?, updated_at = datetime('now') WHERE id = ?
  `),
  addShell: db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `),
  getUserShells: db.prepare(`SELECT * FROM shell_collections WHERE user_id = ? ORDER BY found_date DESC`),
  countUserShells: db.prepare(`SELECT COUNT(*) as count FROM shell_collections WHERE user_id = ?`),
  countUsers: db.prepare(`SELECT COUNT(*) as count FROM users`),
  countPaidUsers: db.prepare(`
    SELECT COUNT(DISTINCT user_id) as count FROM subscriptions WHERE plan != 'free' AND status = 'active'
  `),
};
